...
//   - 日期為純 string（無 marker），要 knex 得 Date 須用 Overrides 逐欄顯式標，未標者兩視角皆 string

import type { Transform as V0_6_1 } from './v0.6.1.js'

// 支援版本清單；採用新版時把版本字串加進聯集，不指定則用 Latest（npm 已發 0.6.0 / 0.6.1，本適配器以 0.6.1 驗證）
...
/** generate-types 的 root（CustomDirectusTypes）→ kit Schema。第二參數填「版本」或直接填「Overrides」（省略版本＝Latest）
 *  - `<Types>` / `<Types, Overrides>` / `<Types, '0.6.1', Overrides>`
 *
 *  Overrides 形狀 `{ collection: { field: 值 } }`，每欄的值二擇一：
 *    - TemporalKind 字面量 → 日期 brand 捷徑（標到的欄位 knex 視角才變 Date、service 仍 string）
