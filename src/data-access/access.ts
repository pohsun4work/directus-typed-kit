// DataAccess 的 runtime 實作：items() / services / service() / knex / getSchema
// 「執行身分」收斂在 resolveAccountability（as → accountability，各值語義見 core/types 的 Identity）
// items() / services 同步回傳 lazy proxy，schema 在方法呼叫時 lazy 取（折進本就 await 的 service 方法、不 stale）
...
import type { SchemaShape, TypedItemsService } from './typed-items.js'
...
export function createDataAccess<S extends SchemaShape>(deps: DataAccessDeps): DataAccess<S> {
  ...
  const lazyService = (
    ...
    new Proxy({} as AnyMethods, {
      get(_target, method) {
        // then 必回 undefined：否則 proxy 成 thenable，誤 await（同步的 services.Foo() 被當 async）
        // 會對 proxy 取 then、觸發假 service 建構且 resolve/reject 永不被呼叫 → 該 await 永久 pending
  ...
  const newItemsService = (
    ...
    return new ItemsService(collection, { knex, accountability, schema })
  }
  ...
  const items = ((collection: string, opts?: { as?: ServiceAs }) =>
    lazyService(
      ...
      opts?.as ?? 'caller',
    ) as unknown as TypedItemsService<S, never>) as unknown as DataAccess<S>['items']
